...
function calculateCommitStreakFromDates(commitDates) {
  ...
  const daysDiff = Math.floor((today - mostRecentCommit) / (1000 * 60 * 60 * 24));
...
function generateRealBadges(userData, reposData) {
  ...
  if (repoCount >= 50) {
    badges.push({ name: "Repository Master", description: "50+ public repositories" });
  } else if (repoCount >= 20) {
    badges.push({ name: "Active Developer", description: "20+ public repositories" });
  } else if (repoCount >= 5) {
    badges.push({ name: "Getting Started", description: "5+ public repositories" });
  }
  ...
  if (languages.size >= 5) {
    badges.push({ name: "Polyglot", description: "Uses 5+ programming languages" });
  }

  // Account age badge
  const accountCreated = new Date(userData.created_at);
  const yearsActive = Math.floor((new Date() - accountCreated) / (1000 * 60 * 60 * 24 * 365));
  if (yearsActive >= 5) {
    badges.push({ name: "Veteran Developer", description: "5+ years on GitHub" });
  } else if (yearsActive >= 2) {
    badges.push({ name: "Experienced", description: "2+ years on GitHub" });
  }

  return badges.length > 0 ? badges : [{ name: "New Developer", description: "Welcome to GitHub!" }];
}
