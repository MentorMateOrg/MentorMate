...
export default function Connections({ targetUser }) {
  ...
  return (
    <>
      ...
      {targetUser && loggedInUserId && targetUser.id !== loggedInUserId && (
      ...
      {/* Show pending connection requests and connections for the current user only. */}
      {(!targetUser ||
        (loggedInUserId && targetUser.id === loggedInUserId)) && (
                      ...
                      <Link to={`/profile/${request.sender.id}`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {request.sender.profile?.role || "User"}
                          </p>
                        </div>
                      </Link>
                    </div>
                ...
                {connections.map((connection) => (
                  ...
                  >
                    <Link to={`/profile/${connection.connectedUser.id}`}>
                      <img
                        src={
                          connection.connectedUser.profile?.profilePicUrl ||
                          "https://static.vecteezy.com/system/resources/previews/055/581/121/non_2x/default-profile-picture-icon-avatar-photo-placeholder-illustration-vector.jpg"
                        }
                        alt="Profile"
                        className="w-12 h-12 rounded-full object-cover"
                      ...
                      <div>
                        <p className="font-medium">
                          {connection.connectedUser.profile?.full_name ||
                            connection.connectedUser.email}
                        </p>
                        <p className="text-sm text-gray-500">
                          {connection.connectedUser.profile?.role || "User"}
                        </p>
